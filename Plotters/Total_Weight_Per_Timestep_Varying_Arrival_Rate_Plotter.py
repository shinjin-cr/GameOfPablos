...
file = open('Total_Weight_Per_Timestep_Varying_Arrival_Rate.pkl', 'rb')
...
plt.ylabel('Total Criminal Weight')

for arrival_parameter in sorted(data.keys()):
    (x, y, z, m, b) = data[arrival_parameter]#(timestep, total_weight as a function of timestep, linear approximation, slope of linear approximation, y-intercept of linear approx)
    shape = shapes.pop(0)
    ...
    plt.plot(xx, yy, shape, label= r'$k = %2d$'%arrival_parameter)
    plt.plot(x, y)
    plt.plot(x, z, '--', label = r'$y =  %1.2f+ %1.2fx$'%(b,m))# m should always be non-negative
    
plt.legend(loc = 'lower right')#fontsize  = 20)
filename_for_pdf = '/'+'Total_Weight_Per_Timestep_Varying_Arrival_Rate.pdf'
