...
file = open('Node_Density_Varying_Arrival_Rate.pkl', 'rb')
...
for arrival_rate in sorted(data.keys()):
    ...
    #OPEN PICKLE FILE#######################################################################
    ND_dict = data[arrival_rate][0]
    ND_data_list = data[arrival_rate][1]
    ...
    color = colors.pop(0)
    plt.plot((heights), (DDen), shapes.pop(0), color = color , label = r'$k = %3d$'%arrival_rate)
...
filename_for_pdf = '/'+'Node_Density_Varying_Arrival_Rate.pdf'
