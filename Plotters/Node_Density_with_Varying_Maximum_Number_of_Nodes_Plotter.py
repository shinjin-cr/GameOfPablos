...
file = open('Node_Density_Varying_Maximum_Number_of_Nodes.pkl', 'rb')
...
for max_allowable_nodes in sorted(data.keys()):
    #OPEN PICKLE FILE#######################################################################
    ND_dict = data[max_allowable_nodes][0]
    ND_data_list = data[max_allowable_nodes][1]
    ...
    color = colors.pop(0)
    plt.plot((heights), (DDen), shapes.pop(0), color = color , label = r'$N = %s$'%str(max_allowable_nodes))
...
filename_for_pdf = '/'+'Node_Density_Varying_Maximum_Number_of_Nodes.pdf'
