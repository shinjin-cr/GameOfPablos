...
#OPEN PICKLE FILE######################################################################
file = open('Height_V_Mean_Degree_Varying_Arrival_Rate.pkl', 'rb')
...
for arrival_parameter in sorted(data.keys()):
    shape = shapes.pop(0)
    height_degree_dict = data[arrival_parameter]
    ...
    plt.plot((heights), (average_degree), shape, alpha = .5, label = r'$k = %3d$'%arrival_parameter)
    #######################################################################################
plt.legend()
filename_for_pdf = '/'+'Height_V_Mean_Degree_Varying_Arrival_Rate.pdf'
