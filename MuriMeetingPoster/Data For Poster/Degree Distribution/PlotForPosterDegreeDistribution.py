...
for max_allowable_nodes in sorted(data.keys()):
    ...
    plt.ylabel(r'Degree Density (log)')#, fontsize = 20)
...
degrees1 = degrees[1:11]
log_degrees = np.log(degree_freq[1:11])
m, b = np.polyfit((degrees1), log_degrees, 1)
lin_approx = m*degrees[1:] + b
plt.plot(degrees[1:], lin_approx, '--',label = r'$y = %1.1f x + %1.1f$'%(m, b))

plt.legend()
filename_for_pdf = '/'+'DDPEAK_poster.pdf'
