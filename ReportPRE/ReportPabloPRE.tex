...
\usepackage{amsthm}  %Theorem Stuff
 ...
 \usepackage{url}
 \usepackage{hyperref}
\usepackage[font = small, labelfont = bf, textfont = small,justification=raggedright ]{caption}
...
\tikzstyle{no2}=[rectangle, draw, fill = yellow!95!green, inner sep= .08 cm]
...
\affiliation{UCLA Department of Mathematics},

 \author{M. Puck Rombach, Andrea L. Bertozzi}
 \affiliation{UCLA Department of Mathematics \\and Institute of Pure and Applied Mathematics}
  \author{Maria R. D'Orsogna}
  \affiliation{UCLA Department of Mathematics \\and CSUN Department of Mathematics}
  \collaboration{MURI Collobaration}
\date{\today}
...
child  {node [no1, label={[label distance=-.25cm]180:$\begin{matrix}\bb{P}_2 \approx\\  0. {09}\end{matrix}$}]{}
       child {node[no2, label=   {[label distance=-0.25cm]-90 :$\begin{matrix}  \bb{P}_2 \approx\\ 0.{18}\end{matrix}$}] {}}
       child {node [no2, label=  {[label distance=-.2cm]0: $\begin{matrix} \bb{P}_2 \approx\\ 0.{18}\end{matrix}$}]{}}}
